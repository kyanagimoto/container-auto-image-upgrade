...
const jqts_1 = __importDefault(__nccwpck_require__(4003));
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const pattern = jqts_1.default.compile('.results | map(.name) | .[]');
...
const error_1 = __nccwpck_require__(7502);
...
const context_1 = __nccwpck_require__(1694);
const __1 = __nccwpck_require__(9679);
const ensure_1 = __nccwpck_require__(7786);
const error_1 = __nccwpck_require__(7502);
const utility_1 = __nccwpck_require__(3152);
const predefined_1 = __nccwpck_require__(7565);
...
function forEachInvocation(ctx, args, fn) {
    ...
    forEachArgs(args.map(arg => arg.evaluate(new context_1.Context(input))), args => fn(input, args)))));
}
...
function arrayFunctionWithKeyEvaluation(fn) {
    return (ctx, args) => {
        ...
        const values = ctx.values.map(input => {
            const values = ensure_1.ensureArray(input);
            const keys = values.map(value => {
                return ensure_1.ensureValue(compareBy.evaluate(new context_1.Context({ value })).values[0]);
            });
...
exports.functions = new Map([
    ...
    [
        'range',
        (input, args) => forEachInvocation(input, args, (_, args) => {
            const [fromOrUpto, uptoOrUndef, byOrUndef] = args.map(ensure_1.ensureNumberValue);
            const [from, upto] = uptoOrUndef === undefined
                ? [0, fromOrUpto]
    ...
    [
        'combinations',
        (ctx, args) => forEachInvocation(ctx, args, (input, args) => {
            predefined_1.combinations;
            const [nOrUndef] = args.map(ensure_1.ensureNumberValue);
...
const context_1 = __nccwpck_require__(1694);
const error_1 = __nccwpck_require__(7502);
const operators_1 = __nccwpck_require__(8302);
const utility_1 = __nccwpck_require__(3152);
...
function join(input) {
    ...
    if (input.length === 0)
        return null;
    return input.reduce((acc, v) => (acc ? operators_1.addOperator(acc, v) : v), undefined);
}
exports.join = join;
...
function range(fromOrUpto, uptoOrUndef, byOrUndef) {
    const [from, upto] = uptoOrUndef === undefined ? [0, fromOrUpto] : [fromOrUpto, uptoOrUndef];
    const by = byOrUndef !== null && byOrUndef !== void 0 ? byOrUndef : 1;
...
function sortBy(values, keys) {
    return keys
        ...
        .sort((a, b) => utility_1.valueCompare(a.key, b.key))
        .map(({ index }) => values[index]);
}
exports.sortBy = sortBy;
function groupBy(values, keys) {
    return [
        ...keys
            .reduce((groups, key, i) => { var _a; return groups.set(key, [...((_a = groups.get(key)) !== null && _a !== void 0 ? _a : []), values[i]]); }, new Map())
            .entries(),
    ]
        .sort((a, b) => utility_1.valueCompare(a[0], b[0]))
        .map(x => x[1]);
}
...
function minIndex(input) {
    ...
    return input.reduce((minIndex, v, i) => utility_1.valueCompare(input[minIndex], v) > 0 ? i : minIndex, 0);
}
...
function unique(input) {
    return [
        ...input.reduce((acc, v) => acc.add(v), new Set()).values(),
    ].sort();
}
exports.unique = unique;
function uniqueBy(values, keys) {
...
function indicesFromString(_input, _target, maxIndices = 0, reverse = false) {
...
function indicesFromArray(_input, _target, maxIndices = 0, reverse = false) {
    ...
    while (start < input.length &&
        ...
        const idx = input
            .slice(start)
            .findIndex(v => utility_1.valueCompare(v, head) === 0);
        if (idx < 0)
            break;
        start += idx;
        if (target.length > 1) {
...
function _indices(input, target, maxIndices = 0, reverse = false) {
    if (isString(input)) {
        if (!isString(target)) {
            throw new error_1.RuntimeError(`a non-string value ${JSON.stringify(target)} cannot be contained in a string`);
        }
    ...
    if (isArray(input)) {
        return indicesFromArray(input, isArray(target) ? target : [target], maxIndices, reverse);
    }
...
const context_1 = __nccwpck_require__(1694);
const functions_1 = __nccwpck_require__(902);
const ensure_1 = __nccwpck_require__(7786);
const error_1 = __nccwpck_require__(7502);
const utility_1 = __nccwpck_require__(3152);
const operators_1 = __nccwpck_require__(8302);
    ...
    evaluate(ctx) {
        try {
            const xs = ctx.values.map(v => {
                ...
                return utility_1.flatten(this.elements
        ...
        try {
            const xs = utility_1.flatten(ctx.values.map(v => {
                ...
                return utility_1.expandCombination(this.fields.map(f => f.evaluate(ctx0)), {}, (prev, vs) => {
                    return vs.map(v => {
                        return Object.assign(Object.assign({}, prev), v);
                    });
                });
            }));
    ...
    evaluate(ctx) {
        try {
            const indexedValues = this.key
                .evaluate(ctx)
                .values.map(key => indexedByKey(ensure_1.ensureObjectKeyOrArrayIndexable(key), ensure_1.ensureValue(ctx.target)));
...
class SliceEvaluator {
    ...
    evaluate(ctx) {
        ...
        return new context_1.Context(...pairs
...
class SpreadEvaluator {
    evaluate(ctx) {
        ...
        if (context_1.isJsonMap(target)) {
            return new context_1.Context(...Object.keys(target)
                .map(k => target[k])
                .map(v => ({ value: v })));
        }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.JQ = void 0;
const scanner_1 = __nccwpck_require__(2314);
const parser_1 = __nccwpck_require__(5281);
const optimize_1 = __nccwpck_require__(1667);
const eval_1 = __nccwpck_require__(9679);
class JQ {
    constructor(evaluator) {
        this.evaluator = evaluator;
    }
...
const stmt_1 = __nccwpck_require__(2042);
const eval_1 = __nccwpck_require__(9679);
const expr_1 = __nccwpck_require__(1610);
const error_1 = __nccwpck_require__(7502);
...
class Scanner {
    ...
    scan(pattern, trim = true) {
        const [matched, ...groups] = typeof pattern === 'string'
